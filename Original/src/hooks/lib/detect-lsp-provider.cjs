...
const PROVIDERS = {
  cclsp: {
    ...
    // First call to make on session start — primes the TS and Python projects
    warmup: { tool: 'get_diagnostics', note: 'primes TS/Python server (cclsp upstream bug #43 workaround). Call on a .ts file AND a .py file to warm both.' },
  },
  serena: {
    label:        'Serena',
...
function detectProviders() {
  ...
  // Serena: primary LSP provider (multi-language: TS, Python, 40+ languages)
  if (mcpNames.has('serena')) {
    active.add('serena');
  }

  // cclsp: fallback — standalone MCP server OR bundled via typescript-lsp plugin
