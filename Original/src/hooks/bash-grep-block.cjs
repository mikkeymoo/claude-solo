...
process.stdin.on('end', () => {
  ...
  const targetsCode =
    /\bsrc[\\/]|\bapp[\\/]|backend[\\/]|components[\\/]|lib[\\/]|hooks[\\/]|utils[\\/]|services[\\/]|actions[\\/]|routers[\\/]/i.test(cmd) ||
    /\.tsx?\b|\.jsx?\b|\.pyi?\b/i.test(cmd) ||
    /-t\s+(ts|tsx|js|jsx|typescript|javascript|py|python)\b/i.test(cmd) ||
    /--type[= ](ts|tsx|js|jsx|typescript|py|python)\b/i.test(cmd) ||
