...
Run `python ~/.claude/skills/mm-cost/cost_report.py` to generate the report. Flags: `--today`, `--week`, `--month`, `--trend`, `--by-project`, `--by-model`, `--top-sessions N`. No flag = full report (all time).
