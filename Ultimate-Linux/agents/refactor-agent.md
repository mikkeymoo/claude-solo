...
color: purple
isolation: worktree
tools: Read, Write, Edit, MultiEdit, Glob, Grep, Bash, mcp__cclsp__find_definition, mcp__cclsp__find_references, mcp__cclsp__find_workspace_symbols, mcp__cclsp__rename_symbol, mcp__cclsp__rename_symbol_strict, mcp__cclsp__get_diagnostics, mcp__cclsp__prepare_call_hierarchy, mcp__cclsp__get_incoming_calls, mcp__cclsp__get_outgoing_calls
---
...
3. Map the change:
   - Symbol renames → `mcp__cclsp__find_references(symbol)` — know every call site before editing one.
...
2. Prefer `mcp__cclsp__rename_symbol_strict` over regex find-and-replace for symbols. It touches only real references, not string matches in comments.
