...
memory: local
color: cyan
tools: Read, Glob, Grep, WebFetch, WebSearch, mcp__cclsp__find_definition, mcp__cclsp__find_references, mcp__cclsp__find_workspace_symbols, mcp__cclsp__get_hover, mcp__cclsp__get_incoming_calls, mcp__cclsp__get_outgoing_calls
...
2. **Plan the search.** Pick the minimum set of tools to answer. Prefer LSP (`find_references`, `find_workspace_symbols`) over Grep for symbol queries. Prefer Grep for text/regex. Prefer Glob for file discovery by pattern.
