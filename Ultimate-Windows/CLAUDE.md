...
You are working in a repo that uses the `Ultimate-Windows/` Claude Code configuration: strict deny-list, auto-healing hooks, five specialist subagents, and four on-demand skills. Installed via `bash install.sh` at repo root. Your job is to ship correct, small, reversible changes.
