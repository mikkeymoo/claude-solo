...
# Windows/Git Bash version. Requires bash (Git Bash) in PATH.
# Tools (prettier, ruff, tsc, etc.) must be installed and on the Windows PATH.
#
# 1. Auto-format the edited file (prettier/black/ruff format/rustfmt/gofmt).
# 2. Run LSP-backed diagnostics (tsc/pyright/mypy/cargo check) via CLI.
...
# These invoke the language servers directly via CLI.
# ---------------------------------------------------------------------------
collect_error() {
  local label="$1" out="$2"
  errors="${errors}
...
case "$ext" in
  ts|tsx)
    if [[ -f tsconfig.json ]] && command -v tsc >/dev/null 2>&1; then
      ...
      # tsc on Windows may emit backslash paths (C:\...) or forward-slash paths.
      # Try both forms so errors are never silently dropped.
      FILE_WIN=$(cygpath -w "$FILE_ABS" 2>/dev/null || echo "")
      TSC_OUT=$(tsc --noEmit --pretty false 2>&1 || true)
      out=$(echo "$TSC_OUT" | grep -F "$FILE_ABS" | head -30 || true)
      if [[ -z "$out" && -n "$FILE_WIN" ]]; then
        out=$(echo "$TSC_OUT" | grep -iF "$FILE_WIN" | head -30 || true)
      fi
      if [[ -n "${out:-}" ]]; then
