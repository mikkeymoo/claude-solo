---
name: mm:riper
description: Enforce Research → Innovate → Plan → Execute → Review phase separation for non-trivial work. TRIGGER when the user requests a feature, refactor, or fix that spans multiple files or has unclear scope. Blocks implementation before research and planning are complete. DO NOT TRIGGER for one-line fixes, typo corrections, or when the user explicitly says "quick" or "just do it".
---
...
## Rules
- You **cannot** skip phases. If the user says "just implement it", remind them this skill was invoked for a reason and offer to switch to quick mode (`/mm:quick`) instead.
