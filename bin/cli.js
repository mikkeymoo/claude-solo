...
async function doInstall(opts) {
  ...
  // rtk installs by default; let users opt out (passes straight to install.sh).
  if (opts.withoutRtk) flags.push("--without-rtk");
...
program
  ...
  .option(
    "-y, --yes",
    "non-interactive: use flag values/defaults, skip prompts",
  )
  ...
  .option("--without-rtk", "skip installing rtk (Rust Token Killer)")
